		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(4); p.AntiBoostStr = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(5); p.GrowthCoefficientStr = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(6); p.OriginalBulletNum = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(7); p.GrowthCoefficientBul = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(8); p.DownPoint = (float)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(9); p.LearningLevel = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(10); p.WaitTime = (float)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(11); p.MoveSpeed = (float)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(12); p.AnimSpeed = (float)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(13); p.Arousal = (float)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(14); p.ReloadType = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(15); p.ReloadTime = (float)(cell == null ? 0 : cell.NumericCellValue);
						s.list.Add (p);
					}
					data.sheets.Add(s);
				}
