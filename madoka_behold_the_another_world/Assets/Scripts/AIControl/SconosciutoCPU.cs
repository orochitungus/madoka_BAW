...
namespace BehaviourTrees
{
	public class SconosciutoCPU : AIControlBase
	{
		...
		protected override bool Engauge(ref KEY_OUTPUT keyoutput)
		{
			...
			if (RockonTarget != null && Vector3.Distance(target.transform.position, RockonTarget.transform.position) <= target.RockonRange)
			{
				...
				else if (!target.IsGrounded)
				{
					...
					// 覚醒発動
					else if(target.Arousal >= target.Arousal_OR)
					{
						keyoutput = KEY_OUTPUT.AROUSAL;
						Shoted = true;
						StartCoroutine(ShotInterval());
		...
		//			}
		//		}
		//		// そうでなければ空中におり、敵との間に遮蔽物がなければ射撃攻撃
		//		else if (!target.IsGrounded)
		//		{
		//			// 前面に向かってレイキャストする
		//			RaycastHit hit;
		...
		//	}
		//	// 射撃のフォロースルーに入ったら再度空中ダッシュさせる
		//	else if (target.AnimatorUnit.GetCurrentAnimatorStateInfo(0).fullPathHash == target.FollowThrowShotID 
		//	|| target.AnimatorUnit.GetCurrentAnimatorStateInfo(0).fullPathHash == target.FollowThrowAirShotID
		//	|| target.AnimatorUnit.GetCurrentAnimatorStateInfo(0).fullPathHash == target.FollowThrowSubShotID
		//	|| target.AnimatorUnit.GetCurrentAnimatorStateInfo(0).fullPathHash == target.FollowThrowEXShotID)
		//	{
		//		tenkeyoutput = TENKEY_OUTPUT.TOP;
		//		keyoutput = KEY_OUTPUT.BOOSTDASH;
		//		Cpumode = CPUMODE.BOOSTDASH;
