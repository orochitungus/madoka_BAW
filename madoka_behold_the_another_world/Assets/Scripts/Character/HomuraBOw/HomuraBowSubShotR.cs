...
public class HomuraBowSubShotR : Bullet
{
	...
	void Update () 
	{
		...
		if (transform.parent != null && !IsIndependence)
		{
			...
			{
				// その時の座標を保持する
				GetComponent<Rigidbody>().position = InjectionObject.GetComponent<HomuraBowControl>().SubShotRootR.GetComponent<Rigidbody>().position;
				// 進行方向ベクトルを保持する
				MoveDirection = InjectionObject.GetComponent<HomuraBowControl>().BulletMoveDirectionR;
				// 攻撃力を保持する
