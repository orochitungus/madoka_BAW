...
namespace Utage
{
	...
	public static class ExcelParser
	{
		...
		//ファイルの読み込み
		public static StringGridDictionary Read( string path, char ignoreSheetMark, bool parseFormula)
		{
			...
			if ( IsExcelFile(path) )
			{
				...
				using (FileStream fs = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
				{
					if (ext == ExtXls)
					{
						ReadBook(new HSSFWorkbook(fs), path, ignoreSheetMark, parseFormula, gridDictionary);
					}
					else if (ext == ExtXlsx)
					{
						ReadBook(new XSSFWorkbook(fs), path, ignoreSheetMark, parseFormula, gridDictionary);
					}
				}
			}
		...
		//ブックの読み込み
		static void ReadBook(IWorkbook book, string path, char ignoreSheetMark, bool parseFormula, StringGridDictionary gridDictionary)
		{
			for (int i = 0; i < book.NumberOfSheets; ++i)
			{
				UnityEngine.Profiling.Profiler.BeginSample("ReadBook");
				ISheet sheet = book.GetSheetAt(i);
				StringGrid grid = ReadSheet(sheet, path, ignoreSheetMark, parseFormula);
